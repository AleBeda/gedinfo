"""`gedinfo descendants` subcommand implementation."""
...
def register(subparsers: argparse._SubParsersAction) -> None:  # type: ignore
    """Register the ``descendants`` subcommand with the top-level parser."""
    sub = subparsers.add_parser(
        "descendants",
        help="List all descendants of an individual",
        description="List all descendants of an individual",
    )
...
def run(args: Any) -> None:
    """Handler invoked when ``gedinfo descendants`` is run."""
    g = args.generations
    ...
    details = get_descendant_details(data, args.indi_id, max_generations=g)
