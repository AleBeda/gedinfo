...
def test_g1_no_output():
    ...
    code, out, err = run_cmd(["givennames", "-g", "1", "@I001@", GED])
    assert code == 0
    assert out.strip() == ""
...
def test_g2_parents_only():
    ...
    code, out, err = run_cmd(["givennames", "-g", "2", "@I001@", GED])
    assert code == 0
    assert "Masculine names:" in out
    assert "Feminine names:" in out
...
def test_invalid_g_zero():
    code, out, err = run_cmd(["givennames", "-g", "0", "@I001@", GED])
    assert code == 1
    assert "Invalid generations value" in err


def test_unknown_id():
    code, out, err = run_cmd(["givennames", "@I999@", GED])
    assert code == 1
...
def test_second_flag():
    ...
    code, out, err = run_cmd(["givennames", "--second", "@I001@", GED])
    assert code == 0
...
def test_second_flag_short():
    ...
    code, out, err = run_cmd(["givennames", "-s", "@I001@", GED])
    assert code == 0
    assert "raphael" in out.lower()
...
def test_hebrew_flag():
    ...
    code, out, err = run_cmd(["givennames", "--hebrew", "@I001@", GED])
    assert code == 0
    assert "מרים" in out
...
def test_hebrew_flag_short():
    ...
    code, out, err = run_cmd(["givennames", "-e", "@I001@", GED])
    assert code == 0
    assert "מרים" in out
...
def test_all_flag():
    ...
    code_a, out_a, _ = run_cmd(["givennames", "-a", "@I001@", GED])
    code_b, out_b, _ = run_cmd(["givennames", "--second", "--hebrew", "@I001@", GED])
    assert code_a == 0
    assert code_b == 0
    assert out_a == out_b
...
def test_fuzzy_groups_variants(tmp_path):
    ...
    code, out, err = run_cmd(["givennames", "-f", "@I001@", str(f)])
    assert code == 0
...
def test_givn_deduplication(tmp_path):
    ...
    f.write_text(content, encoding="utf-8")
    code, out, err = run_cmd(["givennames", "@I001@", str(f)])
    assert code == 0
