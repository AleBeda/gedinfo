...
def test_disjoint_spouse_component_order_preserved():
    ...
    for comp in components:
        lines = render_component(
            data, comp, "both", include_spouse_suppressed=False, include_unknowns=False
        )
        if lines:
            expected_blocks += 1
...
def test_disjoint_spouse_placeholder_when_all_roots_suppressed():
    ...
    lines = render_component(
        data, component, "both", include_spouse_suppressed=False, include_unknowns=False
    )
    assert lines == []
    # With spouse flag: the root is spouse-suppressed but named, so including
    # spouse-suppressed roots will cause it to appear
    lines = render_component(
        data, component, "both", include_spouse_suppressed=True, include_unknowns=False
    )
    assert len(lines) == 1
    assert "I001" in lines[0]
    # With all flags, individuals are included
    lines = render_component(
        data, component, "both", include_spouse_suppressed=True, include_unknowns=True
    )
    assert len(lines) == 1
    assert "I001" in lines[0]
...
def test_disjoint_spouse_output_mode_id_only():
    ...
    code, out, err = run_cmd(
        ["disjoint", "--spouse", "-i", str(FIXTURES / "spouse.ged")]
    )
    assert code == 0
    assert "I001" in out
    assert "I008" in out
...
def test_disjoint_spouse_output_mode_name_only():
    code, out, err = run_cmd(
        ["disjoint", "--spouse", "-n", str(FIXTURES / "spouse.ged")]
    )
    assert code == 0
...
def test_disjoint_spouse_and_conflicting_output_flags():
    code, out, err = run_cmd(
        ["disjoint", "--spouse", "-i", "-n", str(FIXTURES / "spouse.ged")]
    )
    assert code == 1
